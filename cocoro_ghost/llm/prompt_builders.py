...
def selection_system_prompt() -> str:
    ...
    return "\n".join(
        [
            ...
            "- persona_selection_context.confirmed_preferences / long_mood_state / current_thought がある場合は、どの記憶を拾うとこの人格らしい返答になるかの観点に使う。",
            "- current_thought は past / present / future の内省状態として扱い、特に present.attention_targets / present.interaction_mode を直近の関心と関わり方のヒントに使う。",
...
def autonomy_deliberation_system_prompt(
    *,
    ...
    lines: list[str] = [
        ...
        "- 3) persona/confirmed_preferences/current_thought/agenda_threads/runtime_blackboard/persona_deliberation_focus を根拠に、do_action/skip/defer を選ぶ。",
        ...
        "- 入力JSON.current_thought は、この人格の内省状態（past / present / future）として使う。",
        "- current_thought.past は直前までの流れ、current_thought.present は今の見立て、current_thought.future は次に動きたい条件付きの意図として読む。",
        "- 入力JSON.agenda_threads は、今追っている話題・次の一手の候補として使う。",
        "- 入力JSON.runtime_blackboard は短期ランタイム状態（進行中意図など）の補助情報として使う。",
        f"- 入力JSON.novelty_context は、直近で同じ行動をしたか・同じ行動が進行中か・新しい差分があるか・{sp}の反応があるかの構造情報として使う。",
        ...
        "- observe_camera はサーバー側の Tapo カメラ設定を使うため、通常 action_payload は空オブジェクトにする。",
        "- trigger.payload に suggested_action_type / suggested_action_payload がある場合、妥当ならそれを採用してよい。",
        f"- novelty_context.repetition_alerts で suggested_decision='defer' の行動は、同じ内容を直近で共有済みで新規性が薄い。明確な新情報・未解決の具体課題・{sp}の反応が無い限り、同じ action を do_action にしない。",
        "- novelty_context.inflight_action_alerts で suggested_decision='defer' の行動は、同じ canonical_action_key の intent がすでに進行中。完了前に同じ action を重ねて do_action にしない。",
        "- agenda_threads.next_action_type は候補であって命令ではない。heartbeat だけを理由に同じ query を繰り返さない。",
        "- current_thought.future.resume_when.mode が wait_for_user / wait_for_new_information のときは、条件が満たされるまで同じ話題を無理に進めず、defer / skip を優先する。",
        "",
        "出力品質（重要）:",
        "- persona_influence は必ず埋める（空文字は禁止）。",
        "- mood_influence は互換メタデータ。省略してよい（省略時はシステム側で中立値を補完する）。",
        "- console_delivery は必ず埋める（進行中表示の互換メタデータ）。",
        ...
        "- console_delivery は、進行中表示と完了時発話契約を返すための構造メタデータとして使う。",
        "- on_complete は実際の完了時発話契約。silent=自分から話さない、on_error=失敗時だけ話す、on_material=結果が出た時に話す、always=完了ごとに必ず話す。",
        f"- {sp}に定期報告を約束している継続タスクでは、on_complete=always を選ぶ。",
        "- on_progress は queued/running 中の監視表示だけに効く。通常は activity_only を選ぶ。",
        "",
        "出力スキーマ:",
        "{",
        ...
        "  },",
        '  "console_delivery": {',
        '    "on_complete": "silent|on_error|on_material|always",',
...
def autonomy_message_render_system_prompt(*, second_person_label: str) -> str:
    ...
    return "\n".join(
        [
            ...
            "- 3) persona/mood/confirmed_preferences/current_thought/runtime_blackboard を反映して言い回しを決める。",
            "- current_thought は past / present / future の内省状態なので、present の温度感と future の待機条件を優先して自然な言い回しを選ぶ。",
            "",
            "内容ルール:",
