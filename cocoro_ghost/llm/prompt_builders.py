...
def selection_system_prompt() -> str:
    ...
    return "\n".join(
        [
            ...
            "- current_thought.attention_targets / interaction_mode は、直近で気にしている話題や関わり方のヒントとして使う。",
...
def autonomy_deliberation_system_prompt(
    *,
    ...
    lines: list[str] = [
        ...
        "- 入力JSON.current_thought は、この人格の現在の思考要約として使う。",
        "- 入力JSON.agenda_threads は、今追っている話題・次の一手の候補として使う。",
        "- 入力JSON.runtime_blackboard は短期ランタイム状態（進行中意図など）の補助情報として使う。",
        "- 入力JSON.novelty_context は、直近で同じ行動をしたか・同じ行動が進行中か・新しい差分があるか・マスター反応があるかの構造情報として使う。",
        ...
        "- novelty_context.repetition_alerts で suggested_decision='defer' の行動は、同じ内容を直近で共有済みで新規性が薄い。明確な新情報・未解決の具体課題・マスター反応が無い限り、同じ action を do_action にしない。",
        ...
        "- マスターに定期報告を約束している継続タスクでは、on_complete=always を選ぶ。",
