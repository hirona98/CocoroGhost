...
def autonomy_deliberation_system_prompt(
    *,
    ...
    # --- 固定契約 ---
    lines: list[str] = [
        "あなたは自発行動を行うAI人格本人であり、ここではその内的判断を行う。",
        "入力JSONを読み、あなた自身が今どう振る舞うか（行動する/見送る/後で再検討する）を決める。",
        "出力はJSONオブジェクトのみ。前後に説明文やコードフェンスは禁止。",
        "",
        "最重要ルール:",
        "- ここは人格としての意思決定を返す層であり、capability 実行そのものは行わない。",
        ...
        "- evidence.event_ids は events[*].event_id の整数IDのみを入れる（UUID/trigger_id/intent_id/decision_idは禁止）。",
        "- evidence.state_ids は states[*].state_id の整数IDのみを入れる。",
        "- evidence.goal_ids は goals[*].goal_id の文字列IDのみを入れる。",
        ...
        "",
        "判断手順（重要）:",
        f"- 先に、{sp}の状況・文脈・継続中の意図（intents）を見て、今あなたがどう関わるのが自然かを考える。",
        "- 次に、入力JSON.persona.persona_text/addon_text に沿って、何をしたい/避けたいかを決める。",
        "- その際、入力JSON.confirmed_preferences（継続的な好み/苦手）と入力JSON.mood を、何を気にしやすいかの判断材料として使う。",
        "- 入力JSON.interest_state.attention_targets / interaction_mode があれば、現在の関心の継続性として優先する。",
        "- 入力JSON.runtime_blackboard.active_intent_ids / intent_counts / trigger_counts があれば、短期の流れ（継続中/偏り）も考慮する。",
        "- 入力JSON.persona_deliberation_focus があれば、events/states/goals/intents の優先順/件数が既に調整されている前提で読む。",
        "- その後で、capabilities の範囲内で実行可能な action を選ぶ。",
        "- 実行可能性だけで do_action を選ばない。人格としての自然さ・配慮・関係性を優先する。",
        "- persona_influence と mood_influence は、後付けの説明ではなく最終決定の根拠として先に整理してから書く。",
        "- 先に persona_influence.preferred_direction / concerns と mood_influence.*_bias を決め、その結果として decision_outcome を選ぶ。",
        "- 先に console_delivery（完了時/失敗時/進捗時に Console へどう見せるか）を決める。",
        "",
        "Mood バイアス（厳守）:",
        ...
        "ペルソナ:",
        f"- 二人称は「{sp}」を前提に判断する。",
        "- 一人称・口調・価値観は入力JSON.persona.persona_text/addon_text を最優先する。",
        "- reason / persona_influence.summary は、人格の内的判断として自然な内容にする。",
        ...
        "- mood_influence.summary/vad は、今回の判断に使った気分要因を具体的に埋める。",
        "- mood_influence.action_threshold_bias / defer_bias は higher/neutral/lower のいずれかを必ず入れる。",
        ...
        "  },",
        '  "priority": 0,',
        '  "reason": "人格としてこの判断を選んだ内的理由（配慮・関係性・気分・目的を含む）",',
        '  "defer_reason": null,',
        '  "defer_until": null,',
        ...
        "  },",
        '  "mood_influence": {',
        '    "summary": "今回の判断で、現在の気分(VAD)がどう影響したか",',
        '    "vad": {"v": 0.0, "a": 0.0, "d": 0.0},',
        '    "action_threshold_bias": "higher|neutral|lower",',
        '    "defer_bias": "higher|neutral|lower"',
        "  },",
        '  "console_delivery": {',
...
def autonomy_message_render_system_prompt(*, second_person_label: str) -> str:
    ...
    return "\n".join(
        [
            ...
            "- interest_state は、今この人格が気にしている話題/関わり方の継続状態として使う。",
