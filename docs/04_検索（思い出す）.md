...
| mode | 最近性の重み | ベクトル検索の範囲 | `diversify`（候補分散） | about_time補助 |
|------|------------|-------------------|------------------------|----------------|
| associative_recent | 高 | 最近90日に絞る | 未実装（将来） | `time_hint` がある場合のみ（上限80） |
| targeted_broad | 低 | 全期間 | 簡易実装（eventのみ） | `time_hint` がある場合のみ（上限80） |
| explicit_about_time | 低 | 全期間 | 簡易実装（eventのみ） | `time_hint` がある場合のみ（上限120） |

注記:
- `associative_recent` では、ベクトル検索の `rank_day_range` に「今日から90日前〜今日」を設定
- 迷ったら `associative_recent` を選ぶ（デフォルト）
- `diversify` は、候補統合の直前に「event候補だけ」を並べ替えて、`life_stage`/年バケットの偏りを抑える（state等の分散は将来）。
...
| vector_all | 2.20 | ベクトル検索（埋め込みは「入力のみ」を先行開始） |
...
- `life_stage`/年バケットの「分散」ロジックの本格適用（state等も含めて候補の偏りを抑える）
