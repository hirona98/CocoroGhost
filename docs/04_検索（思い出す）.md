...
- **stateリンク展開（1-hop）**: `state_links`（seed→state_links→関連state）
  - seed の state 候補から、state_links を1回だけ辿って関連 state を追加する
  - 多段化はノイズが増えるため行わない（1-hopのみ）
...
- 「固有名詞/型番」: 文字n-gram
- 「あの時どう思ってた？」: `event_affect`（瞬間的な感情）+ 長期の `long_mood_state`
...
- 長期運用で `state` が増えすぎてノイズ化する場合は、非同期で整理（過去化）を行う（`docs/05_記憶更新（育てる）.md` の「記憶整理（tidy_memory）」）
...
- `retrieval_state_link_expand_*`: stateリンク展開（seed→state_links→関連state）の上限/足切り
...
- `event_affects` は行を残しつつ、`vec_items` を削除して想起対象から外す（内部用・削除しない）
