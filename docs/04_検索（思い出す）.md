...
| targeted_broad | 低 | 全期間 | 未実装（将来） | `time_hint` がある場合のみ（上限80） |
| explicit_about_time | 低 | 全期間 | 未実装（将来） | `time_hint` がある場合のみ（上限120） |

注記:
- `associative_recent` では、ベクトル検索の `rank_day_range` に「今日から90日前〜今日」を設定
- 迷ったら `associative_recent` を選ぶ（デフォルト）
- （未実装）`SearchPlan.diversify` は plan には含まれるが、現状は候補統合/上限制御へ反映していない（将来の候補分散用の予約）。
