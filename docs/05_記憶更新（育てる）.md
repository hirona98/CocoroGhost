...
- 実装責務は `cocoro_ghost/worker.py`（ジョブスケジューリング）と `cocoro_ghost/worker_handlers.py`（ディスパッチ）および `cocoro_ghost/worker_handlers_*.py`（jobs.kind の実処理）で分離している
- WritePlan 系は `cocoro_ghost/worker_handlers_write_plan_generate.py`（生成）と `cocoro_ghost/worker_handlers_write_plan_apply.py`（適用）に分割している
