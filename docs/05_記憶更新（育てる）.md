...
- 実装責務は `cocoro_ghost/jobs/runner.py`（ジョブスケジューリング）と `cocoro_ghost/jobs/registry.py`（ディスパッチ）および `cocoro_ghost/jobs/handlers/*.py`（jobs.kind の実処理）で分離している
- WritePlan 系は `cocoro_ghost/jobs/handlers/write_plan_generate.py`（生成）と `cocoro_ghost/jobs/handlers/write_plan_apply.py`（適用）に分割している
