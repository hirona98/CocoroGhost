...
- 「人名/組織/場所/プロジェクト/話題」などを軸に、LLM選別前の候補を増やしつつ、取りこぼしを減らす。
- 多段推論をLLMの幻覚に頼らず、「展開ルール」で作る。

設計案（CocoroGhost向け）:

- **正規化キー**: `entity:<type>:<slug>`（例: `entity:person:amanda`）のような形で統一。
- **保存場所（推奨）**:
  - `events.entities_json`（既存）を正として維持
  - 検索用に別途「entity ↔ event/state」の参照を持つ（専用テーブル or `state.payload_json` の標準フィールド）
- **同期の候補収集に追加**:
