...
- 候補に `confidence/importance`（概念）を揃えて持たせる。
  - `WritePlan` の `confidence` を将来的にランキングに使えるよう、候補JSONにも入れる。
...
- 実装済み（2026-01-25）
  - 保存: `state_links` テーブル（`docs/06_ストレージ（SQLite）.md`）
  - 生成: 非同期 `build_state_links` ジョブ（WritePlan適用後に投入、LLM判定）
  - 検索: 同期候補収集に `state_link_expand`（1-hop）を追加（`docs/04_検索（思い出す）.md`）
  - 設定:
    - `memory_state_links_build_*`（生成側の上限/閾値）
    - `retrieval_state_link_expand_*`（検索側の展開上限/閾値）
...
3) B-1（state↔stateリンク生成）※実装済み  
4) B-2（スレッド/話題summaryのメタ記憶化）  
5) C-1（必要ならPPR型ランキング）
