...
- Web検索（インターネット）は `/api/chat` の最終応答生成（L3）でのみ有効化可能（`llm_preset.reply_web_search_enabled`）。自律実行能力（例: `web_access`）とは別制御: `docs/10_実行フロー.md` / `cocoro_ghost/llm_client.py` / `docs/20_階層型世界モデル/00_概要.md`
