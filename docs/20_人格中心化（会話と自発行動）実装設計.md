...
- 本文中の `persona_interest_state` は、現在は `current_thought_state` と `agenda_threads` に置き換わっている。
- 本文中の `console_delivery` は、現在は **進行中表示用メタデータ** として扱う。
- 完了時の即時発話は `silent / chat` で決まり、軽い共有優先度は `talk_impulse` に保持する。
- 以下は設計経緯の記録として残しており、読むときは上記に読み替える。
...
- `ActionDecision.console_delivery` は `do_action` 時に必須とする
  - 本文の意味解析ではなく、表示方針（`silent/activity_only/chat` 等）の構造情報を持たせる
...
- `current_thought_state` と `agenda_threads` を導入
...
3. 完了時の即時発話判定が `chat` の場合のみ人格発話生成を実行
...
初期は `ConfirmedPreferences` と `current_thought`（未導入なら空）から構築する。
...
- `state.kind = "current_thought_state"`（要約）
- `agenda_threads`（機械側の正本）
...
- `deliberate_once` 完了後（必要な構造情報だけを反映）
...
- 即時発話がある場合は、常に人格の通常発話として表示する
- 軽い共有は `talk_impulse` に保持し、通知UIへは流さない
...
- `cocoro_ghost/llm/prompt_builders.py`
  ...
  - `report_focus` 優先（`result.summary_text` は補助）ルールを追加
...
- `cocoro_ghost/jobs/handlers/autonomy.py`
  ...
  - `autonomy.message` 再生成入力へ `report_focus`（構造化焦点候補）を追加
  - `result_payload.details.raw_output_text` を本文生成入力から除外（meta化）
- `cocoro_ghost/llm/client.py`
  - `LlmRequestPurpose.ASYNC_AUTONOMY_MESSAGE_RENDER`（必要なら）
- `cocoro_ghost/autonomy/agent_runner.py`
  - `details_json.raw_output_text` の扱い整理（推奨）
- `CocoroConsole/Services/CommunicationService.cs`
  - `autonomy.message` は通知UIへ流さず、通常発話だけを扱う
- `cocoro_ghost/jobs/handlers/write_plan_generate.py`
...
- `cocoro_ghost/memory/_chat_mixin.py`
  ...
  - `compact_candidates` の入力順を `current_thought`（`interaction_mode` / `attention_targets`）の構造重みで調整
- `cocoro_ghost/llm/prompt_builders.py`
  - `selection_system_prompt()` を人格対応に拡張
- `cocoro_ghost/jobs/handlers/autonomy.py`（または新規builder）
  ...
  - 実装済み（第一段階）:
    ...
    - `current_thought.interaction_mode` を Deliberation の `interaction_mode_hint` に反映
    - `current_thought.attention_targets`（`world_*` 含む）の重みを `intent/goals` 優先度へ反映
    - `runtime_blackboard.attention_targets`（RAM）を ActionResult から更新し、snapshot_payload にも保存
    - 文字列比較による意味推定は使わない
  - 未実装（次段階）:
    - `current_thought_state / agenda_threads` に基づく attention_targets の重み最適化（会話側の候補収集/選別まで含む）

### Phase C

- `cocoro_ghost/jobs/handlers/write_plan_apply.py`（または専用worker）
  - `current_thought_state / agenda_threads` 更新
  - 実装済み（第一段階）:
    - `apply_write_plan` で `current_thought_state` を単一更新
    - `apply_write_plan` で `agenda_threads` を更新
    - 構造情報（entities/preference更新/state更新/event source/client_context/world_model_items）から `attention_targets` を構築
    - `runtime_blackboard.attention_targets` にも反映
- `cocoro_ghost/storage/memory_models.py`
  - 既存 `states` の `kind` 拡張で足りるなら新テーブル不要
- `cocoro_ghost/llm/prompt_builders.py`
  - 会話/自発向け prompt で `current_thought` を参照
- `cocoro_ghost/memory/_chat_mixin.py`
  - `current_thought_state` snapshot を会話の選別/返答 internal context に注入
- `cocoro_ghost/jobs/handlers/autonomy.py`
  - Deliberation入力 / autonomy.message 再生成入力に `current_thought` を注入
...
- `do_action` の ActionDecision に `console_delivery` が必ず含まれる（進行中表示用）
...
- Deliberation の材料選別で `persona/current_thought/attention` が使われる
  - 第一段階: `confirmed_preferences/current_thought/runtime_blackboard` に基づく並び順・件数調整
  - 次段階: `current_thought_state / agenda_threads` を使った注目対象選別の重み最適化（world_model_items は関心状態更新材料に反映済み）

### Phase C

- `current_thought_state` と `agenda_threads` が継続更新される
