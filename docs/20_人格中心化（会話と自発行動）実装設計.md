...
- `cocoro_ghost/memory/_chat_mixin.py`
  ...
  - `compact_candidates` の入力順を `interest_state`（`interaction_mode` / `attention_targets`）の構造重みで調整
- `cocoro_ghost/prompt_builders.py`
...
- `cocoro_ghost/worker_handlers_autonomy.py`（または新規builder）
  ...
  - 実装済み（第一段階）:
    ...
    - `interest_state.interaction_mode` を Deliberation の `interaction_mode_hint` に反映
    - `interest_state.attention_targets`（`world_*` 含む）の重みを `intent/goals` 優先度へ反映
    - `runtime_blackboard.attention_targets`（RAM）を ActionResult から更新し、snapshot_payload にも保存
    - 文字列比較による意味推定は使わない
  - 未実装（次段階）:
    - `persona_interest_state` に基づく attention_targets の重み最適化（会話側の候補収集/選別まで含む）
...
- `cocoro_ghost/worker_handlers_write_plan_apply.py`（または専用worker）
  ...
  - 実装済み（第一段階）:
    - `apply_write_plan` で `persona_interest_state` を単一更新
...
- `cocoro_ghost/memory/_chat_mixin.py`
  - `persona_interest_state` snapshot を会話の選別/返答 internal context に注入
- `cocoro_ghost/worker_handlers_autonomy.py`
  - Deliberation入力 / autonomy.message 再生成入力に `interest_state` を注入
...
- Deliberation の材料選別で `persona/mood/attention` が使われる
  ...
  - 次段階: `persona_interest_state` を使った注目対象選別の重み最適化（world_model_items は関心状態更新材料に反映済み）
