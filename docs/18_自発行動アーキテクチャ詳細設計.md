...
- `cocoro_ghost/memory/_external_mixin.py`
  - `run_desktop_watch_once` / `run_reminder_once` の意思決定ロジックを撤去
  - 既存メソッドは「トリガ受理」または「Capability呼び出し」へ分割
...
| `cocoro_ghost/worker_handlers_write_plan_generate.py` | ActionDecision/ActionResult入力取り込み |
