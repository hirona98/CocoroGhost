...
- 入力: Trigger + ContextPack + Persona + CurrentThought + AgendaThreads + RuntimeBlackboard + NoveltyContext + CapabilityCatalog
- 出力: ActionDecision（構造化JSON）
- 責務:
  - `do_action / skip / defer` の決定
  - 実行理由と証拠の明示
  - Persona影響と thread 選定根拠の明示
  - 同一行動の直近反復（新情報なし / ユーザー反応なし）を検出し、最初から `skip/defer` を選べるようにする
...
- `console_delivery_json` は進行中表示用であり、完了時の `silent/chat` は `action_result -> talk_impulse` から決める
...
| `canonical_action_key` | TEXT NOT NULL | 同一行動判定キー（正規化） |
...
- 部分一意制約を持つ
  - `UNIQUE(canonical_action_key) WHERE status IN ('queued','running','blocked')`
  - 同じ行動は、進行中の間は常に1件だけにする

Intent 生成規約（必須）:

- `decision_outcome='do_action'` の ActionDecision から Intent を作る時は、まず `canonical_action_key` を決める
- 同じ `canonical_action_key` の live intent（`queued/running/blocked`）がある場合、新しい Goal / Intent は作らず既存 intent を再利用する
- 新規作成時は `decision_id` で冪等、かつ active `canonical_action_key` と衝突したら再利用に倒す
- 生成SQLは次を正とする  
  - `INSERT OR IGNORE INTO intents (...) VALUES (...)`
- `IGNORE` の場合は既存 live intent を読み出して継続し、新規 Intent を再生成しない
...
| `talk_impulse_level` | TEXT NOT NULL | `none/low/high/speak_now` |
| `talk_impulse_reason` | TEXT NULL | 発話衝動の理由 |
...
- `current_thought_state` は「人間の内省状態」を表す制御状態とし、`past / present / future` の三段で保持する
- `current_thought_state` は通常記憶の検索・リンク・埋め込み対象に混ぜず、`agenda_threads` は機械側の正本として扱う
...
- `action_result` 由来の `world_query` から、同一 `web_research` を即時再予約しない
  - 調査完了そのものを理由に heartbeat ループを作らない
  - 継続調査は、新しい外部変化・明示的な追加要求・未解決タスクがある時だけ再度 `next_action_type` を持つ
...
- `current_thought`: `current_thought_state`（専用入力。`past / present / future` の内省状態）
- `agenda_threads`: 直近の `active/open/blocked` thread（専用入力）
- `runtime_blackboard`: RAM上の短期注目状態
- `novelty_context`: 同一行動の直近実行回数 / 内容差分 / 進行中 intent / ユーザー反応有無 / クールダウン対象
...
|---|---|---|
| `vision_perception` | `observe_screen` / `observe_camera` | screen target / Tapo camera settings |
...
- `attention_targets`（action_result 由来の短期刺激のみ）
- `policy_timers`
...
- `current_thought_state` は「直前まで何をして、今どう見えていて、次に何をしたいか」を表す内省状態としてサーバ側で再構成する
- `current_thought_state` は LLM の通常 `state_updates` では更新させず、`agenda_threads` は機械側の正本として更新する

### 完了時の共有判定

- 進行中表示は `ActionDecision.console_delivery_json` で扱う
- 完了時の `silent / chat` は `action_result -> talk_impulse` で決める
- `ActionDecision.agenda_thread_id` を使い、実行結果は exact な `agenda_thread` に反映する
- `delivery_decision` は debug 上で、現在の発話衝動を持つ thread の要約として観測する
...
- `tapo_camera_host` (text)
- `tapo_camera_username` (text)
- `tapo_camera_password` (text)
- `tapo_camera_cloud_password` (text)
