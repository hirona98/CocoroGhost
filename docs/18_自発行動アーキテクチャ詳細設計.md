...
- Persona と Mood（VAD）を自発行動の必須入力として実行時に効かせる
...
<!-- Block: ImplementationStatus20260222 -->
## 実装進捗（2026-02-22 時点）

本書は「設計の正」を保持する文書であり、以下はその設計に対する**現行実装の到達点**を示す進捗メモである。

### 実装済み（主要）

- autonomy 基本ループ（trigger claim -> `deliberate_once` -> `execute_intent` -> `action_result`）
- `deliberation_decision` / `action_result` の専用テーブル保存と会話想起ノイズ制御
- `/api/chat` 経路と autonomy Web検索経路の分離（`web_access` capability）
- `runtime_snapshots` の定期保存（30秒）と起動時復元（active intent 再キュー）
- `runtime_blackboard` の導入（snapshot payload と同期）
- `reminder` の autonomy 連携を `event` ではなく `time` trigger で投入
- `desktop_watch` / `camera_watch` / `time_routine` の Policy trigger 投入
- `vision_perception` / `web_access` / `schedule_alarm` capability 実装
- `world_model_items` への ActionResult 集約（現行は主に `web_access`）
- `world_model_items` -> `state.fact` の deterministic 昇格判定（`generate_write_plan`）
- `goals` の最小運用（ActionDecision 由来で生成、成功時 `done`）

### 一部未達 / 継続課題

- `cocoro_ghost/autonomy/context_builder.py` / `deliberation.py` / `executor.py` への責務分割は未実施
  - 現在は `cocoro_ghost/worker_handlers_autonomy.py` に集約されている
- `device_control` / `mobility_move` は capability の入口のみ実装（現時点は構造化 `failed` を返す）
- `run_desktop_watch_once` 本体は legacy 実行ヘルパとして残存
  - ただし `desktop_watch.py` からの直呼びは廃止済みで、`vision_perception` capability からのみ利用する
- `intents.status=blocked` の遷移実装は未整備（集計/表示は先行）
- ActionDecision の生JSONは `action_decisions` が正だが、現行実装では `events.client_context_json` にも監査用として併記している
...
flowchart TD
    ...
    MOOD["Mood Snapshot (LongMood + VAD)"] --> DLB
    ...
    EVR --> WP
    WP --> ST["state / preferences / context graph / affect"]
    ST --> CTX
...
- 入力: Trigger + ContextPack + Persona + Mood + CapabilityCatalog
...
- 主はイベント駆動、補助として heartbeat と時刻tick
- 役割:
  - Triggerキュー管理
  - Intent実行順序管理
  - 再計画条件の投入
...
- `cocoro_ghost/memory/_external_mixin.py`
  - `run_reminder_once` は reminder event の保存/配信責務に限定する（autonomy trigger 投入は `reminders_service.py` 側）
  - `run_desktop_watch_once` は legacy 実行ヘルパとして維持し、`vision_perception` capability から呼ぶ
- `cocoro_ghost/desktop_watch.py`
  - 直接実行から、`desktop_watch_policy` の tick 呼び出しへ変更
- `cocoro_ghost/reminders_service.py`
...
| `do_action` | INTEGER NOT NULL | 0/1 |
...
| `mood_influence_json` | TEXT | VAD影響要約 |
...
- `CHECK (decision_outcome = 'do_action' OR do_action = 0)`
- `CHECK (decision_outcome <> 'do_action' OR do_action = 1)`
...
- `mood`: `LongMoodState` + 直近 `event_affects`
...
<!-- Block: MoodPersonaBias -->
## Mood/Persona バイアス適用仕様

### 入力

- Persona: `persona_text` を trait化した固定セット
- Mood:
  - `long_mood_state.vad`
  - 直近 `event_affects` の移動平均

### 判定規則

- `A` が低い時は新規行動開始閾値を上げる
- `V` が低い時は回避系 action を優先
- `D` が低い時は `defer` / `confirm` を優先

### 監査

- ActionDecision に `persona_influence` と `mood_influence` を必須保存
...
| ファイル | 変更内容 |
|---|---|
| `cocoro_ghost/llm_client.py` | Deliberation purpose追加、会話Web経路と自発Web経路を分離 |
| `cocoro_ghost/memory_models.py` | autonomy系テーブル追加 |
| `cocoro_ghost/db.py` | 新テーブルindex追加、user_version更新 |
| `cocoro_ghost/memory/_chat_search_mixin.py` | `deliberation_decision` 除外 |
| `cocoro_ghost/worker_handlers_write_plan_generate.py` | ActionDecision/ActionResult入力取り込み、`world_model_items` 昇格判定（deterministic） |
| `cocoro_ghost/worker_handlers_autonomy.py` | Deliberation/Execution、本体保存、`world_model_items` 集約、goal最小運用 |
...
| `cocoro_ghost/autonomy/capabilities/*.py` | `vision_perception`/`web_access`/`schedule_alarm` 実装、`device_control`/`mobility_move` 入口 |
| `cocoro_ghost/desktop_watch.py` | Policy化（直接判断撤去） |
| `cocoro_ghost/reminders_service.py` | クライアント接続依存除去、時刻トリガ投入 |
| `cocoro_ghost/camera_watch.py` | camera_watch の periodic service（Policy trigger 投入） |
...
<!-- Block: AcceptanceStatus20260222 -->
## 受け入れ条件の現状評価（2026-02-22 時点）

- `/api/chat` を経由せず `web_research` が完走し、decision/resultが追跡できる
  - 実装済み（`web_access` capability + `action_decisions` / `action_results` 保存）
- `source=deliberation_decision` が会話想起に混入しない
  - 実装済み（会話検索で source 除外）
- UI未接続でも time trigger による intent 実行が進む
  - 基盤実装済み（`reminder` は `time` trigger で投入）
  - `time_routine` 自体の行動内容は Deliberation/Capability の判断に依存
- 再起動後、`runtime_snapshots` から active intents を復元できる
  - 実装済み（snapshot 復元 + execute 再キュー）
- 常時観測時に trigger/intents/jobs の滞留がAPIで観測できる
  - 概ね実装済み（`/api/control/autonomy/status` + `/api/control/worker-stats` を併用）
  - 観測頻度の自動減速（負荷制御）は未実装
