...
- 単一ユーザー運用前提のため、Web UI 上で API キー等を含む設定JSONを直接編集する
...
- 右上の設定アイコン（⚙）で設定画面を開く
- 設定画面は `/api/settings` の JSON をそのまま表示し、再読込・編集・保存を行う
- 保存は `PUT /api/settings` の全量更新で反映する
- 保存成功時は、サーバから返された最新JSONでエディタを上書きする（サーバ正を表示）
- 設定画面内からログアウト可能にする
