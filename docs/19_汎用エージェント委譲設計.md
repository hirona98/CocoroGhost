...
Phase 1 の `agent_runner` は、backend 実行を **subprocess の標準入出力JSON** で扱う。

- runner -> backend subprocess（stdin）
  - `job_id`, `backend`, `task_instruction`, `intent_id`, `decision_id`, `created_at`
- backend subprocess -> runner（stdout）
  - 成功JSONのみ（`result_status`, `summary_text`, `details_json`）
- 失敗時
  - subprocess を非ゼロ終了し、`stderr` にエラーを書く（runner は `/fail` callback）

stdout JSON（成功）の最小契約:

```json
{
  "result_status": "success|partial|no_effect",
  "summary_text": "string",
  "details_json": {}
}
```
...
- `Codex` 固定ではなく、`--backend-command backend_name='command ...'` で外部backendを接続する
...
- `cocoro_ghost/worker_handlers.py` / 新規 `worker_handlers_agent_jobs.py`（stale sweepを入れる場合）
