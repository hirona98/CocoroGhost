...
- `cocoro_ghost/jobs/handlers/autonomy.py` (`_collect_deliberation_input`)
- `cocoro_ghost/llm/prompt_builders.py`（`agent_delegate` の出力例追記）
...
- `cocoro_ghost/jobs/handlers/autonomy.py` の action_type 分岐
...
- `cocoro_ghost/api/control.py` / `cocoro_ghost/api/services/control_service.py`（既存 `/api/control/*` に追加）
...
4. `api/control.py` + `api/services/control_service.py` + `schemas.py` に `claim/heartbeat/complete/fail/list/get` 追加
...
- `cocoro_ghost/storage/memory_models.py`（`agent_jobs` 追加）
- `cocoro_ghost/storage/db.py`（memory DB version更新）
- `cocoro_ghost/jobs/handlers/autonomy.py`（`agent_delegate` 分岐 / callback最終化ヘルパ）
- `cocoro_ghost/llm/prompt_builders.py`（Deliberation 出力例）
- `cocoro_ghost/api/control.py` / `cocoro_ghost/api/services/control_service.py`（`/api/control/agent-jobs/*`）
