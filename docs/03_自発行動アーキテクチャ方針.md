...
## 実装進捗メモ（2026-02-22 時点）

この文書は方針固定を目的とするため、設計内容そのものは維持しつつ、現行実装の到達点だけを簡潔に記す。
詳細な実装差分・受け入れ条件の評価は `docs/18_自発行動アーキテクチャ詳細設計.md` を参照する。

### 到達した項目（概要）

- autonomy 基本ループ（trigger / deliberation / intent execution / result 保存）
- `runtime_snapshots` の定期保存と起動時復元
- `runtime_blackboard` の導入
- `reminder` の `time` trigger 化
- `desktop_watch` / `camera_watch` / `time_routine` の Policy trigger 化
- `web_access` / `vision_perception` / `schedule_alarm` capability 実装
- `world_model_items` 集約と `state.fact` への deterministic 昇格（WritePlan）

### 継続課題（概要）

- `worker_handlers_autonomy.py` の責務分割（`context_builder` / `deliberation` / `executor`）
- `device_control` / `mobility_move` の実機統合
- 観測頻度の自動減速を含む負荷制御の高度化
...
- 定期実行基盤: `cocoro_ghost/periodic.py`
...
- Policy/Service 実装: `cocoro_ghost/desktop_watch.py`, `cocoro_ghost/camera_watch.py`, `cocoro_ghost/reminders_service.py`, `cocoro_ghost/autonomy/policies/`
- Capability 実装: `cocoro_ghost/autonomy/capabilities/`
- イベント配信: `cocoro_ghost/event_stream.py` (`/api/events/stream`)
- 記憶更新基盤: `cocoro_ghost/worker.py`, `cocoro_ghost/worker_handlers_*.py`, `cocoro_ghost/worker_handlers_autonomy.py`
- 記憶モデル: `cocoro_ghost/memory_models.py` (`events`, `state`, `revisions`, ...)
