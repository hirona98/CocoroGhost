...
- `cocoro_ghost/app_bootstrap/`: 起動時の設定/DB初期化、router登録、startup/shutdown 配線
- `cocoro_ghost/api/services/`: API ルーターから切り出した運用ロジック
- `cocoro_ghost/jobs/runner.py`: 非同期ジョブ（埋め込み更新/WritePlan適用/整理など）
- `cocoro_ghost/jobs/`: `jobs.kind` と実処理ハンドラの登録表
- `cocoro_ghost/reminders/`: リマインダーのDB/モデル/計算/サービス
- `cocoro_ghost/runtime/`: ログ/イベント配信、定期実行、ロギング設定
- `cocoro_ghost/llm/prompt_builders.py`: system/user/internal context のプロンプト組み立て
- `cocoro_ghost/core/affect.py`: VAD と LongMoodState（baseline+shock）の更新/減衰ロジック
- `cocoro_ghost/core/common_utils.py`: JSON安定ダンプ、[face:*]除去、LLMのJSON抽出などの共通処理
- `cocoro_ghost/storage/vector_index.py`: vec_items の kind と item_id 生成ルール
