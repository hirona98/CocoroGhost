...
## shared client_id 設計（全クライアントで会話共有）

要求:

- ネイティブ/ブラウザを含めて会話を共有する（同じ client_id として扱う）

方針:

- サーバ側に `shared_client_id` を 1つ定義する
- `/api/chat` のリクエストに含まれる `client_id` は採用せず、常に `shared_client_id` を使う
- `/api/events/stream` の `hello.client_id` も同様に `shared_client_id` に上書きして登録してよい

同時送信:

- 初期は運用で避ける（同時送信すると会話が混ざり得る）
- 必要になったら `shared_client_id` 単位の排他（409 など）を追加する
...
- FastAPI で静的配信する（例: `/` が `index.html` を返す）
