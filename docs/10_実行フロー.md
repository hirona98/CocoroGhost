...
- **Web検索（インターネット検索）は最終応答生成（L3）でのみ有効化可能（LLMプリセット設定ON時）**
...
- Web検索（インターネット）は **会話応答作成（L3）だけ** で有効化する（記憶検索経路には混ぜない）
- Web検索のON/OFFは `llm_preset.reply_web_search_enabled` で制御する
- 上記は `/api/chat` 経路の制約であり、自律実行能力（例: `web_access`）の設計とは分けて扱う
...
## 自律ループ（worker/periodic, 仕様）

本セクションは `docs/20_階層型世界モデル/` を正とする仕様。
`/api/chat` の同期フローとは別に、階層型世界モデルを worker/periodic 側で実行する。

### サイクルの流れ（要点）

1. トリガ投入（`event_created` / `action_result` / `periodic` / `startup`）
   - 実行ジョブは `jobs.kind="run_autonomy_cycle"` を使用する
2. 5ループ実行（Observe -> Strategize -> Tacticalize -> Execute -> Reflect）
3. command event 配信（必要時）
   - 例: `vision.capture_request`
   - `events` には保存せず、`events/stream` の `event_id=0` で扱う
4. result event 永続化
   - 例: `meta_proactive` / `vision_detail` / `desktop_watch` / `autonomy_action`
5. result event から WritePlan ジョブを投入
   - `upsert_event_embedding` -> `upsert_event_assistant_summary` -> `generate_write_plan`
6. World Model（`wm_*`）を更新し、次サイクルへ再利用する

### 競合回避（重要）

- 自律ループ処理は `/api/chat` 同期経路で待たない
- 同期経路のSSE開始待ち時間を増やさない
- Web検索（インターネット）の会話応答用フラグ（`reply_web_search_enabled`）と
  自律 capability 実行は別制御のまま維持する
